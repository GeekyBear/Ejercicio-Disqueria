...
entity Musicians : cuid, managed {
    ...
    last_name  : String(25);
    band       : Composition of many Musicians_Bands
                     on band.musicians = $self;
    session    : Composition of many Sessions
                     on session.musician = $self;
}

entity Bands : cuid, managed, name {
    genre     : MusicGenre;
    musicians : Composition of many Musicians_Bands
                    on musicians.band = $self;
    records   : Association to many Records
                    on records.band = $self;
}
...
entity Records : cuid, managed, name {
    ...
    distribution : Composition of many Records_Distribution
                       on distribution.record = $self;
    session      : Composition of many Sessions
                       on session.record = $self;
    amount       : Integer;
}

entity Distribution : cuid, managed, name {
    address : String(25);
    record  : Composition of many Records_Distribution
                  on record.distribution = $self;
}
